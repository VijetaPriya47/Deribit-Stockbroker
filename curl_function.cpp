...
string auth() {
    ...
    // API endpoint URL
    string url = "https://test.deribit.com/api/v2/public/auth?client_id="+clientId +"&client_secret="+clientSecret+"&grant_type=client_credentials&timestamp=1734024793444";
...
void buyOrder(string instrument_name,int amount, double price) {
   ...
   Json::Value orderData;
    ...
    // Print results
    std::cout << "Order Placement Latency: " << latency.count() << " ms" << endl;
...
void ticker(string instrument_name) {

    // Measure latency
    auto start_time = std::chrono::high_resolution_clock::now();

    string url = "https://test.deribit.com/api/v2/public/ticker?";

   Json::Value orderData;

    // Adding fields to the JSON
    orderData["method"] = "private/ticker";
    ...
    if (curl) {
        ...
        if (res == CURLE_OK) {
            cout << "Ticker Data: " << responseData << endl;
        } else {
            cerr << "cURL Error: " << curl_easy_strerror(res) << endl;
        }
